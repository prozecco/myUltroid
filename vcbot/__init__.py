...
async def get_spotify_metadata(url):
    match = re.search(r'(?:spotify:track:|spotify\.com/(?:embed/)?track/)([a-zA-Z0-9]+)', url)
    if not match:
        return None
    ...
    try:
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(embed_url, headers=headers, timeout=10) as res:
                if res.status != 200:
                    return None
                text = await res.text()
        
        match_json = re.search(r'<script[^>]*id=["\']__NEXT_DATA__["\'][^>]*>(.*?)</script>', text, re.DOTALL)
        if not match_json:
            return None
...
async def download(query):
    # Spotify Support
    if "spotify.com" in query.lower() or query.startswith("spotify:"):
        meta = await get_spotify_metadata(query)
        if meta:
            search = VideosSearch(meta["query"], limit=1).result()
            if search and search.get("result"):
                data = search["result"][0]
                link = query
                title = meta["name"]
                duration = time_formatter(meta["duration_ms"])
                thumb = meta["thumb"]
                dl = await get_stream_link(data["link"])
    ...
    if "soundcloud.com" in query.lower():
        try:
            import json
            out = await bash(f'yt-dlp -j "{query}"')
            info = json.loads(out[0])
            title = info.get("title") or query
            duration = time_formatter(int(info.get("duration", 0)) * 1000) if info.get("duration") else "Unknown"
            ...
            LOGS.exception(e)
            dl = await get_stream_link(query)
            return dl, None, query, query, "Unknown"
...
async def get_stream_link(ytlink):
    ...
    return k
    """
    if "soundcloud" in ytlink.lower():
        fmt = "bestaudio/best"
    else:
        fmt = "best[height<=?720][width<=?1280]"
    stream = await bash(
        f'yt-dlp -g -f "{fmt}" --js-runtimes node --remote-components ejs:github "{ytlink}"'
    )
    return stream[0]


async def vid_download(query):
    # Spotify Support for video play
    if "spotify.com" in query.lower() or query.startswith("spotify:"):
        meta = await get_spotify_metadata(query)
        if meta:
            search = VideosSearch(meta["query"], limit=1).result()
            if search and search.get("result"):
                ...
                duration = time_formatter(meta["duration_ms"])
                thumb = meta["thumb"]
                video = await get_stream_link(data["link"])
                return video, thumb, title, link, duration

    # SoundCloud Support for video play
    if "soundcloud.com" in query.lower():
        try:
            import json
            out = await bash(f'yt-dlp -j "{query}"')
            ...
            LOGS.exception(e)
            video = await get_stream_link(query)
            return video, None, query, query, "Unknown"
